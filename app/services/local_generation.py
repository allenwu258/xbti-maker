...
class LocalGenerationProvider:
    """Deterministic fallback generator used for local development and UI testing."""
    ...
    async def stream_generate(self, brief: ThemeBrief) -> AsyncIterator[dict]:
        steps = [
            f"正在拆解主题「{brief.topic}」并提炼可传播的人格母题。",
            f"按 {brief.dimension_count} 个维度组织结构，确保每个维度都有清晰的高低描述。",
            f"按 {brief.question_count} 道题展开题库，同时准备 {brief.result_count} 个结果人格模板。",
        ]
        for step in steps:
            yield {"type": "reasoning", "delta": step + "\n"}
            await asyncio.sleep(0.02)

        config = self.generate_from_brief(brief)
        payload = config.json(ensure_ascii=False, indent=2)
        for index in range(0, len(payload), 48):
            yield {"type": "output", "delta": payload[index : index + 48]}
            await asyncio.sleep(0.005)
        yield {"type": "completed", "config": config, "raw_output": payload, "provider": "local"}
